...
- 内部来源：主循环中的三个 Subgraph 节点分别来自 internal 模板 `buddy_request_intake`、`buddy_capability_loop` 和 `buddy_final_reply`。当前协议仍要求 Subgraph 节点嵌入 `config.graph`，因此主循环保存的是模板内容副本；契约测试会校验这些嵌入副本与 internal 模板保持一致。
