...
基础 RAG 的图结构可以表达。当前 state 仍支持 `knowledge_base`，但旧的内置 `search_knowledge_base` skill 和相关专门 validator 约束已删除；RAG 需要通过 `skill/<skill_key>` 文件夹提供自定义检索 skill，并在 Agent Node 中显式绑定 input/output mapping。
