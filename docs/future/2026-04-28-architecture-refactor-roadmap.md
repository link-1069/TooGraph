...
- `runtime/run_artifacts.py` 已承接 run artifact refresh、exported output saved-file matching、state snapshot refresh 和 run snapshot append/deep-copy behavior；旧的 knowledge summary 兼容层已移除，`core/langgraph/runtime.py` 已直接依赖新模块。
...
- `runtime/node_handlers.py` 已承接 input、condition 和 agent node handler body，包括 input output construction、condition branch execution、skill invocation ordering、stream callback wiring、generation kwargs、output value projection、warning dedupe 和 final result selection；旧的 knowledge-base skill 特判已移除，`node_system_executor.py` 保留 `_execute_input_node`、`_execute_agent_node` 与 `_execute_condition_node` 兼容门面。
