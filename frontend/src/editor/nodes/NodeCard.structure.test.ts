...
test("ToolNodeBody renders deterministic tool selection with managed ports", () => {
  ...
  assert.match(toolNodeBodySource, /selectedToolInputFields/);
  assert.match(toolNodeBodySource, /isStaticInputEnabled/);
  assert.match(toolNodeBodySource, /updateStaticInputMode/);
  assert.match(toolNodeBodySource, /commitStaticInputDraft/);
  assert.match(toolNodeBodySource, /class="tool-node-body__static-inputs"/);
  assert.match(toolNodeBodySource, />Card value</);
  assert.match(toolNodeBodySource, /<ElSwitch[\s\S]*class="tool-node-body__static-mode-switch"[\s\S]*:model-value="isStaticInputEnabled\(field\.key\)"/);
