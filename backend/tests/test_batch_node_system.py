...
def test_delegation_worker_batch_workflow_template_executes_subgraph_workers_and_merges(monkeypatch) -> None:
    ...
    result = execute_node_system_graph_langgraph(graph, save_final_run=False, emit_lifecycle_events=False)
    package = result["state_values"]["worker_merge_review_package"]

    assert package["kind"] == "worker_merge_review_package"
    assert package["status"] == "partial"
    assert package["worker_count"] == 2
    assert package["status_counts"] == {"failed": 1, "succeeded": 1}
    assert [item["task_id"] for item in package["outputs"]["findings"]["values"]] == [
        "worker_eval_research_1"
    ]
    assert package["review"]["recommended_next_action"] == "tighten_budget_or_split_task"
    assert "worker_budget_exhausted:worker_eval_research_2:max_steps" in package["review"]["risk_flags"]
    assert "worker_budget_exhausted:worker_eval_research_2:max_chars" in package["review"]["risk_flags"]
    assert "worker_retried:worker_eval_research_1:2" in package["review"]["risk_flags"]
    assert package["review"]["budget_exceeded"] == [
        {"task_id": "worker_eval_research_2", "limit": "max_chars", "used": 4500, "max": 4000},
        {"task_id": "worker_eval_research_2", "limit": "max_steps", "used": 3, "max": 2},
    ]
    assert package["review"]["retry_attempts"] == [{"task_id": "worker_eval_research_1", "attempts": 2}]
    ...
    assert len(child_runs) == 2
    assert {run["invocation_kind"] for run in child_runs} == {"batch_subgraph_worker"}
    assert {run["batch_group_id"] for run in child_runs} == {"run_worker_batch"}

    batch_execution = next(item for item in result["node_executions"] if item["node_id"] == "run_worker_batch")
    assert batch_execution["artifacts"]["batch"]["max_concurrency"] == 2
    assert batch_execution["artifacts"]["batch"]["worker_source"] == "subgraph"
