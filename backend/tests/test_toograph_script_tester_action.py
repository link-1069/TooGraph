...
SCRIPT_TESTER_ACTION_DIR = REPO_ROOT / "action" / "official" / "toograph_script_tester"
SCRIPT_TESTER_MANIFEST_PATH = SCRIPT_TESTER_ACTION_DIR / "action.json"
SCRIPT_TESTER_BEFORE_LLM_PATH = SCRIPT_TESTER_ACTION_DIR / "before_llm.py"
SCRIPT_TESTER_AFTER_LLM_PATH = SCRIPT_TESTER_ACTION_DIR / "after_llm.py"
...
class TooGraphScriptTesterActionTests(unittest.TestCase):
    def test_manifest_exposes_script_testing_contract(self) -> None:
        definition = _parse_native_action_manifest(SCRIPT_TESTER_MANIFEST_PATH, ActionSourceScope.OFFICIAL).definition
        ...
        requirements = (SCRIPT_TESTER_ACTION_DIR / "requirements.txt").read_text(encoding="utf-8")
        self.assertIn("pytest", requirements)

    def test_before_llm_injects_system_and_test_authoring_context(self) -> None:
        payload = _run_action_script(SCRIPT_TESTER_BEFORE_LLM_PATH, {"graph_state": {"script_source": "def add(a, b): return a + b"}})
